// ABOUTME: Integration tests for the POST /api/send-newsletter route handler.
// ABOUTME: Validates auth, issue fetching, Beehiiv draft creation, and error handling.
...
const mockIssue = {
  ...
  subtitle: { en: "Welcome", es: "Bienvenido" },
  nicosTransmission: {
    en: [
      {
        _type: "block",
...
const mockIssueEnOnly = {
  ...mockIssue,
  title: { en: "EN Only Issue" },
  nicosTransmission: {
    en: [
      {
        _type: "block",
...
beforeEach(async () => {
  ...
  const mod = await import("@/app/api/send-newsletter/route.js");
  POST = mod.POST;
...
function makeRequest(body, { withAuth = true } = {}) {
  ...
  return new Request("https://crashlog.ai/api/send-newsletter", {
    method: "POST",
    headers,
...
describe("POST /api/send-newsletter", () => {
  ...
  it("returns 401 when Authorization header has wrong secret", async () => {
    const request = new Request("https://crashlog.ai/api/send-newsletter", {
      method: "POST",
      headers: {
  ...
  it("returns 404 when issue not found in Sanity", async () => {
  ...
  it("returns 502 when Sanity fetch fails", async () => {
    ...
    const response = await POST(makeRequest({ slug: "crash-log-001" }));
  ...
  it("creates EN-only draft when no ES translation exists", async () => {
    sanityClient.fetch.mockResolvedValueOnce(mockIssueEnOnly);
  ...
  it("creates both EN and ES drafts when full translation exists", async () => {
    sanityClient.fetch.mockResolvedValueOnce(mockIssue);
    mockFetch
  ...
  it("sends correct payload to Beehiiv", async () => {
    ...
    await POST(makeRequest({ slug: "crash-log-001" }));
    ...
    expect(callBody.title).toContain("The Crash Log #001");
