...
export function buildEmailHtml(issue, locale = "en") {
  ...
  const sourceLabel = locale === "es" ? "Fuente" : "Source";
  let stackTraceHtml = "";
  if (stackTrace.length > 0) {
    const items = stackTrace
      .map((hit) => {
        const bodyBlocks = t(hit.text, locale);
        const bodyHtml = portableTextToHtml(bodyBlocks);
        const sources = hit.sources || [];
        const sourcesHtml = sources
          .map((s) => {
            const outlet = escapeHtml(s.outlet || s.title || "");
            const url = s.url || "";
            return url
        ...
        return `
          <tr><td style="padding: 12px 0; border-bottom: 1px solid #2a2a30;">
            <div style="font-family: 'Source Serif 4', Georgia, serif; font-size: 15px; color: #c8cdd6; line-height: 1.6; margin: 0 0 4px;">${bodyHtml}</div>
            ${sourcesHtml ? `<p style="font-family: 'IBM Plex Mono', monospace; font-size: 11px; color: #6b6b72; margin: 4px 0 0;">${sourceLabel}: ${sourcesHtml}</p>` : ""}
          </td></tr>`;
      })
      .join("");
    ...
    <tr><td align="center">
      <table width="720" cellpadding="0" cellspacing="0" style="max-width: 720px; width: 100%;">
        ...
        <tr><td style="padding: 32px 28px 0;">
          ...
          ${subtitle ? `<p style="font-family: 'Source Serif 4', Georgia, serif; font-size: 18px; font-style: italic; color: #9aa0aa; line-height: 1.65; margin: 0 0 32px;">${subtitle}</p>` : ""}
        </td></tr>
